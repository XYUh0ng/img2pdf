...
- 👆 **滑动切图** — 移动端未放大时左右滑动切换图片，放大时拖拽平移，像素级跟手
- 🔄 **弹窗内交换** — 底部工具栏显示页数，支持与前后页交换顺序
- 📋 **Ctrl+V 粘贴** — PC 端支持直接粘贴剪贴板图片
- ⬇️ **一键导出** — 按当前排序生成 A4 尺寸 PDF，支持可选图片压缩
- 💾 **数据持久化** — 任务和图片数据自动保存，关闭浏览器后重新打开不丢失
- 🧠 **任务记忆** — 自动记住上次选中的任务，重新打开时恢复状态
- ✏️ **任务名防重名** — 新建任务自动编号，修改时实时检测重复
...
- 移动端弹窗支持滑动切图、双指缩放、单指拖拽，触摸事件已做手势锁隔离（v1.4.0+）
